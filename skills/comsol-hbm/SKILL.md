...
**COMSOL 을 돌리기 전에 반드시 메모리 모드를 사용자에게 물어라.** 플래그를 사용자가
기억하게 하지 말고, 환경을 재서 권장안을 제시하고 고르게 한다.

먼저 가용 메모리를 확인한다:

```bash
free -g | awk '/^메모리|^Mem/{print "  총 " $2 " GB / 가용 " $7 " GB"}'
grep -qi microsoft /proc/version && echo "  WSL 환경 (호스트 RAM 의 일부만 할당됨)"
```

그 다음 AskUserQuestion 으로 묻는다. 기준은 이렇다.

| 가용 메모리 | 권장 | 근거 |
|---|---|---|
| 20 GB 이상 | 원본 정밀도(2차) | 실측 물리메모리 16.5 GB |
| 20 GB 미만 | 저메모리(1차) | 2차는 못 들어간다 |

선택지는 3개로 제시한다:
- **저메모리 (1차 이산화)** — 자유도 약 7.5배 감소, 16GB 급에서 유일한 선택.
  정확도는 2차보다 낮으므로 메시 수렴 게이트로 확인해야 한다고 함께 알린다.
- **원본 정밀도 (2차 이산화)** — 실측 16.5 GB 필요. 최종 확정값용.
- **둘 다** — 저메모리로 먼저 빠르게 보고, 원본 정밀도로 재확인. 시간이 넉넉할 때.

사용자가 고르면 그에 맞춰 실행한다:

```bash
bash $PLUGIN/scripts/solve_comsol.sh --lowmem --params ...   # 저메모리
bash $PLUGIN/scripts/solve_comsol.sh --params ...            # 원본 정밀도
```

`--lowmem` 은 이산화를 1차로 낮춘 변형본을 자동 생성해 쓴다. 반영 여부는 스크립트가
자유도로 자동 대조하고, 무시됐으면 경고 후 `exit 4` 한다
(그때는 GUI 에서 물리현상 > 이산화 > 온도: 선형 으로 직접 바꿔야 한다).

**탐색 루프 자체는 실측 651 MB 이하라 이 질문과 무관하다.** 저사양에서도 그대로 돌린다.
질문은 COMSOL 배치 직전에만 한다.
